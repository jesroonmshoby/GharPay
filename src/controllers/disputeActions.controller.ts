...
export const submitCourtApplicationHandler = async (
  ...
  try {
    ...
    // Must have reached round 3 OR mediator review OR unresolved state
    ...
    if (!isEligibleForCourt) {
      res.status(400).json({
        ...
        error: {
          ...
          message: 'Court Case Registration Application becomes available after 3 negotiation rounds complete without agreement or during Mediator Review.',
        },
      });
      return;
    }
